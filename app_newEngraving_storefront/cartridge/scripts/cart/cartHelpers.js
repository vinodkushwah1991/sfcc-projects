...
try {
    productLineItem.custom.vEngraving = options[0].engravingMsg;
} catch (error) {
...
function getExistingProductLineItemInCart(product, productId, productLineItems, childProducts, options) {
    var matchProduct =  getExistingProductLineItemsInCart(product, productId, productLineItems, childProducts, options) ;
    if(options[0].engravingMsg){
        var result = matchProduct.filter(function(item){
            return  item.custom.vEngraving === options[0].engravingMsg ;
         });
    return result[0];
    }
